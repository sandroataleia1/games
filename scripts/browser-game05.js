...
try {
  ...
  for (const [prompt, right, wrong] of [["Pergunta um", "Certa um", "Errada um"], ["Pergunta dois", "Certa dois", "Errada dois"]]) { await page.getByLabel("Enunciado").fill(prompt); await page.getByLabel("Duração").fill("5"); await page.getByLabel("Alternativa 1").fill(right); await page.getByLabel("Alternativa 2").fill(wrong); await page.getByRole("button", { name: "Adicionar pergunta" }).click(); await page.getByRole("status").filter({ hasText: "Alterações salvas" }).waitFor(); }
  await page.getByRole("button", { name: "↑" }).last().click(); await page.getByRole("button", { name: "Publicar" }).click();
  await page.getByRole("link", { name: "← Painel" }).click(); await page.getByRole("link", { name: "Criar sala" }).click();

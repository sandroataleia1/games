...
export default function Home() {
  ...
  useEffect(() => {
    const client = createLobbyClient({ onStatusChange: async (next) => { setStatus(next); if (next === "connected") { try { const me = await api("/auth/me"); setUser(me.user); const response = await client.command(EVENTS.QUIZ_LIST); if (response?.ok) { setQuizzes(response.data.quizzes); setQuizId((current) => current || response.data.quizzes[0]?.id || ""); } else setError(response?.error?.message || "Não foi possível carregar os quizzes."); } catch { setUser(null); } } } });
  ...
  const canCreate = status === "connected" && user && quizId && quizzes.length > 0;
  return <main className={styles.shell}><section className={styles.board} aria-labelledby="title">
    <header className={styles.header}><div className={styles.brand}><span className={styles.mark} aria-hidden="true">Q</span><span>QuizArena</span></div><div>{user ? <Link href="/painel">Painel de {user.name}</Link> : <Link href="/login">Entrar para criar sala</Link>}</div><div role="status" aria-live="polite" className={`${styles.status} ${styles[status]}`}><span className={styles.statusDot} aria-hidden="true" />{statusLabels[status]}</div></header>

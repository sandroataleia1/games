...
const statusFor = (code) => ({ UNAUTHENTICATED: 401, INVALID_CREDENTIALS: 401, NOT_FOUND: 404, EMAIL_CONFLICT: 409, CONFLICT: 409, TRANSACTION_CONFLICT: 409, QUIZ_IN_USE: 409, QUIZ_NOT_DRAFT: 409, QUIZ_INVALID: 422, INVALID_ORDER: 422, INVALID_PAYLOAD: 422, RATE_LIMITED: 429 }[code] ?? 400);
...
const wrap = (handler) => async (request, response) => { try { await handler(request, response); } catch (error) { const code = error instanceof DomainError ? error.code : "INTERNAL_ERROR"; response.status(code === "INTERNAL_ERROR" ? 500 : statusFor(code)).json(errorBody(error)); } };

export function installApi(app, { database, webOrigin, production = false }) {
  const attempts = new Map();
  function limited(request) { const key = request.ip; const now = Date.now(); const values = (attempts.get(key) ?? []).filter((time) => now - time < 60000); values.push(now); attempts.set(key, values); if (values.length > 10) throw new DomainError("RATE_LIMITED"); }
  ...
  app.post("/api/auth/register", mutation, wrap(async (req, res) => { limited(req); const result = await database.organizers.register(req.body); setSession(res, result); res.status(201).json({ ok: true, data: { user: result.user } }); }));
  app.post("/api/auth/login", mutation, wrap(async (req, res) => { limited(req); const result = await database.organizers.login(req.body); setSession(res, result); res.json({ ok: true, data: { user: result.user } }); }));
